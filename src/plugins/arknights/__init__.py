...
async def terminology_func(message: Message = CommandArg()):
    term_name_list = message.extract_plain_text().strip().split()
    if not term_name_list:
        await terminology.finish(help_str)

    term_name_to_id = {term.term_name: term.term_id
                       for term in game_data.raw_data.excel.gamedata_const.term_description_dict.values()
                       if term.term_name in term_name_list}
    ...
    for term_name in term_name_list:
        ...
        lines.append("")
        if term_name not in term_name_to_id:
            term_description = f"未找到术语 {term_name} 的释义"
        else:
            term_description = game_data.raw_data.excel.gamedata_const.term_description_dict[term_name_to_id[term_name]].description
        lines.append(term_description)
    initial_queue = list(term_name_to_id.values())
    term_id_list = get_term_ids(initial_queue)
    for term_id in term_id_list:
