...
@Rule
def with_command_start_or_to_me(
...
chat_command_group = MatcherGroup(
    rule=with_command_start_or_to_me, block=False, priority=5
)
chat = chat_command_group.on_command(
    "chat", aliases=set(models.keys()), force_whitespace=True
)
...
async def chat_func(
    ...
    if reply_segments := event.original_message["reply"]:
        ...
        if reply_message_id in sessions:
            ...
            await chat.send(
                "您回复的消息可能不是 Chat 的回答，已改为发起新的会话。\n"
                "如果您认为这是一个错误，请联系管理员。"
            )

    answer, session = await session.get_reply(
        message_plain_text, models.get(command_str, "qwen-max")
    )
    if answer["code"] == 0:
        content = answer["data"]["content"]
        content = (
            f"您可以回复任意一条 Chat 的回答，并发送“#{command_str} <下一个问题>”，可以从回复的地方继续对话。\n"
        ...
        await chat.finish(
            f"出现错误，错误码：{answer['code']}，错误信息：{answer['message']}，请联系管理员。",
            reply_message=True,
        )
