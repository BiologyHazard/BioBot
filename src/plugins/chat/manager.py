...
async def get_reply(text: Text, model: str = "qwen-max", **kwargs) -> Result:
    ...
    match model:
        ...
        case "spark":
            return await spark_get_reply(
                spark_appid, spark_api_key, spark_api_secret, spark_version, text
            )
        case _:
...
class Session:
    def __init__(
        self, text_part: TextPart | None = None, parent: Session | None = None
    ) -> None:
    ...
    def add_message(
        self, role: Literal["system", "user", "assistant"], message: str
    ) -> Self:
    ...
    async def get_reply(
        self, message: str, model: str = "qwen-max", cut: int | bool = True
    ) -> tuple[Result, Self]:
        # text = {
        #     'role': 'user',
        #     'content': question,
