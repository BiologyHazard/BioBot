...
def format_event(kind: str, event: WebhookEvent) -> str:
    """按 webhook 类型分发到对应 formatter；未知类型属于调用错误。"""
    if kind == "push":
    ...
    raise ValueError(f"Unsupported webhook event: {kind}")
