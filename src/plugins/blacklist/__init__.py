...
add_all_friend = blacklist_command_group.on_command('拉黑所有好友', aliases={'屏蔽所有好友'})
del_all_friend = blacklist_command_group.on_command('解禁所有好友', aliases={'解封所有好友'})
reset_blacklist = blacklist_command_group.on_command('重置黑名单', aliases={'清空黑名单'})
ban_auto_sleep = blacklist_command_group.on_command('自觉静默', permission=SUPERUSER)


def save_blacklist() -> None:
    file_path.write_text(json.dumps(blacklist, ensure_ascii=False, indent=4), encoding='utf-8')
...
@add_userlist.handle()
async def add_user_list(event: MessageEvent, arg: Message = CommandArg()) -> None:
...
async def add_group_list(arg: Message = CommandArg()) -> None:
    msg = handle_msg(arg, 'add', 'grouplist')
    await add_grouplist.finish(msg)


@del_userlist.handle()
async def del_user_list(event: MessageEvent, arg: Message = CommandArg()) -> None:
    if uids := [at.data['qq'] for at in event.get_message()['at']]:
...
async def del_group_list(arg: Message = CommandArg()) -> None:
...
async def reset_list(flag: str = ArgStr('flag')) -> None:
