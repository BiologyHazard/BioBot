...
__plugin_meta__ = PluginMetadata(name="tygj", description="", usage="tygj？tyg114514")
...
async def tyg_query_func(event: GroupMessageEvent) -> None:
    ...
    global inst
    if inst is None or datetime.date.fromtimestamp(
        event.time
    ) != datetime.date.fromtimestamp(inst.time):
        inst = None
        await tyg_query.finish("我不到啊，问群友罢（")

    await tyg_query.finish(
...
@tyg_change.handle()
async def tyg_change_func(
    event: GroupMessageEvent, group: Annotated[tuple[str, str], RegexGroup()]
) -> None:
    if not Tygj.in_business_hours(event.time):
        await tyg_query.finish("tyg闭店中")
    ...
    if not signature:
        ...
        if tyg_num - num_change < 0:
            await tyg_change.finish("tyg的人数不能小于0")
        tyg_num -= num_change
    inst = Tygj(
        num=tyg_num,
