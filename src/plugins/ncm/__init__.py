...
"""功能设置"""
music_regex = on_regex("(song|url)\?id=([0-9]+)(|&)", priority=2, block=False)
"""歌曲id识别"""
playlist_regex = on_regex("playlist\?id=([0-9]+)&", priority=2, block=False)
"""歌单识别"""
...
async def music_reply_receive(
    ...
    if info["type"] == "song" and await song_is_open(event):
        ...
        await bot.send(
            event=event, message=info["lmsg"] + "\n下载中,上传时间较久,请勿重复发送命令"
        )
        not_zips = await nncm.download(
            ids=info["ids"], lid=info["lid"], is_zip=ncm_config.ncm_playlist_zip
        )
