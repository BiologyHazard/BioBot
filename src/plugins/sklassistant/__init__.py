...
async def extract_uid(*, skland: SKLand, matcher: Matcher, qq: int, uid: str | None, app_code: str = "arknights") -> dict[str, Any]:
    """
...
async def skl_missing_characters_func(matcher: Matcher, event: MessageEvent, message: Message = CommandArg()) -> None:
    skland = SKLand()
    binding_character = await extract_uid(skland=skland,
                                          matcher=matcher,
                                          qq=event.user_id,
