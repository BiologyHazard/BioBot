...
async def 森空岛获取信息(token: str, uid: str | None = None) -> dict[str, Any]:
    ...
    if uid is None:
        uid = characters[
            "defaultUid"
        ]  # TODO: 如果没设置默认角色，就没有 defaultUid 这个字段
...
async def 森空岛实时数据分析(token: str, uid: str | None = None) -> str:
    ...
    信息内容: str = (
        f"Dr. {数据['status']['name'].split('#')[0]}，\n"
        "根据森空岛的目前采集到的信息推算，罗德岛需要注意的情况向您汇报如下：\n"
    )

    # 理智
    当前理智: float = (
        数据["status"]["ap"]["current"]
        + (数据["currentTs"] - 数据["status"]["ap"]["lastApAddTime"]) / 360
    )
    理智回满剩余时间: float = (
        数据["status"]["ap"]["completeRecoveryTime"] - 数据["currentTs"]
    )  # 秒
    if 理智回满剩余时间 < 0:
    ...
    公开招募刷新次数填充时间: float = (
        数据["building"]["hire"]["completeWorkTime"] - 数据["currentTs"]
    )  # 秒
    if 公开招募刷新次数填充时间 < 0:
        公开招募刷新次数 += 1
    if 公开招募刷新次数 > 3:
    ...
    无人机充满剩余时间: float = (
        数据["building"]["labor"]["lastUpdateTime"]
        + 数据["building"]["labor"]["remainSecs"]
        - 数据["currentTs"]
    )
    if 无人机充满剩余时间 < 0:
    ...
    线索交流剩余时间 = max(
        0, 数据["building"]["meeting"]["clue"]["shareCompleteTime"] - 数据["currentTs"]
    )
    if 可赠线索 > 5:
        信息内容 += f"可赠线索 {可赠线索} 个\n"
    if 线索交流剩余时间 == 0:
    ...
    for 疲劳干员 in 数据["building"]["tiredChars"]:
        if 疲劳干员["charId"] == "char_285_medic2":
            承曦格雷伊在发电站: bool = any(
                干员["charId"] == "char_1027_greyy2"
                for 房间 in 数据["building"]["powers"]
                for 干员 in 房间["chars"]
            )
            Lancet_2在发电站: bool = any(
                干员["charId"] == "char_285_medic2"
                for 房间 in 数据["building"]["powers"]
                for 干员 in 房间["chars"]
            )
                ...
                for 干员 in 房间["chars"]:
                    ...
                    if 基建类目 == "dormitories":
                        ...
                        if 干员["charId"] == "char_300_phenxi":
                            干员心情 = min(
                                干员心情
                                + (数据["currentTs"] - 干员["lastApAddTime"]) / 1800,
                                24,
                            )
                        if 干员心情 > 17:
                            刺玫 = False
...
async def 森空岛干员阵容查询(token: str, uid: str | None = None) -> str:
    ...
    数据: dict[str, Any] = obj["data"]
    阵容内容: str = (
        f"Dr. {数据['status']['name'].split('#')[0]} 博士，\n"
        ...
        if 干员["evolvePhase"] == 2:
            for 模组 in 干员["equip"]:
                ...
                if 模组["level"] == 3:
                    ...
                    not 数据["equipmentInfoMap"][模组["id"]]["typeIcon"] == "original"
                    and 模组["id"] == 干员["defaultEquipId"]
                ):
                    模组是开的 = True
                    if 数据["charInfoMap"][干员["charId"]]["rarity"] == 3:
                        消耗龙门币 += 20000
...
def divide(
    x, y, /, *, positive=float("inf"), negative=float("-inf"), zero=float("nan")
) -> float:
    try:
        return x / y
...
async def skl_assistant_func(player_info: PlayerInfo, verbose: bool) -> str:
    ...
    if (
        当前理智 < player_info.data.status.ap.max
    ):  # 如果理智未满，则考虑信息刷新延迟期间的理智恢复
        当前理智 += (
            player_info.data.current_ts - player_info.data.status.ap.last_ap_add_time
        ) // 360
    ...
    lines.append(
        f"当前理智 {当前理智}，{format_time(player_info.data.current_ts, player_info.data.status.ap.complete_recovery_time, CST)}回满。"
    )
    lines.append("")
    ...
    if player_info.data.building.hire is not None:
        ...
        if (
            player_info.data.building.hire.complete_work_time > 0
            and player_info.data.current_ts
            >= player_info.data.building.hire.complete_work_time
        ):
            公开招募刷新次数 += 1
        ...
        if player_info.data.building.hire.complete_work_time <= 0:
            ...
            lines.append(
                f"当前公开招募可刷新 {公开招募刷新次数} 次，{format_time(player_info.data.current_ts, player_info.data.building.hire.complete_work_time, CST)}填充次数。"
            )
        lines.append("")
    ...
    if (
        当前无人机 < player_info.data.building.labor.max_value
    ):  # 如果无人机未满，则考虑信息刷新延迟期间的无人机恢复
        当前无人机 += (
            player_info.data.current_ts
            - player_info.data.building.labor.last_update_time
        ) * divide(
            player_info.data.building.labor.max_value
            - player_info.data.building.labor.value,
            player_info.data.building.labor.remain_secs,
            positive=0,
            negative=0,
            zero=0,
        )
    ...
    lines.append(
        f"当前无人机 {int(当前无人机)} 个，{format_time(player_info.data.current_ts, 无人机充满时间戳, CST)}充满。"
    )
    lines.append("")
    ...
    if player_info.data.building.meeting is not None:
        ...
        lines.append(
            f"可赠线索 {可赠线索} 份，{format_time(player_info.data.current_ts, player_info.data.building.meeting.complete_work_time, CST)}刷新。"
        )
        lines.append("")
    ...
    if player_info.data.building.meeting is not None:
        if player_info.data.building.meeting.clue.share_complete_time <= 0:
            ...
            lines.append(
                f"线索交流{format_time(player_info.data.current_ts, player_info.data.building.meeting.clue.share_complete_time, CST, seconds=True)}结束。"
            )
        lines.append("")
    ...
    if player_info.data.building.tired_chars:
        char_name_list: list[str] = [
            player_info.data.char_info_map[tired_char.char_id].name
    ...
    rooms: list[
        Power | Manufacture | Trading | Dormitory | Meeting | Hire | Control
    ] = []
    ...
    for room in rooms:
        for char in room.chars:
            if char.char_id == 菲亚梅塔_id:
                心情 = min(
                    24,
                    char.ap / 360000
                    + 2 / 3600 * (player_info.data.current_ts - char.last_ap_add_time),
                )
                lines.append(
                    f"{player_info.data.char_info_map[char.char_id].name}的心情为 {心情:.2f}"
                )
                if 心情 == 24:
                    alerts.append("菲亚梅塔心情已满！")
            if char.char_id in {令_id, 夕_id}:
                lines.append(
                    f"{player_info.data.char_info_map[char.char_id].name}的心情为 {char.ap / 360000:.2f}"
                )
    lines.append("")

    # 剿灭作战
    if (
        player_info.data.campaign.reward.current
        < player_info.data.campaign.reward.total
        and (
            datetime.fromtimestamp(player_info.data.current_ts, tz=CST)
            - timedelta(hours=4)
        ).weekday()
        >= 5
    ):  # 鹰历周六、周日
        alerts.append("剿灭没打完！")
    lines.append(
        f"剿灭作战每周报酬：{player_info.data.campaign.reward.current} / {player_info.data.campaign.reward.total}"
    )
    ...
    if verbose:
        ...
        # 基本信息
        lines.append(
            f"Dr. {player_info.data.status.name}（{player_info.data.status.uid}），声望 Lv. {player_info.data.status.level}，入职于 {datetime.fromtimestamp(player_info.data.status.register_ts, tz=CST).strftime('%Y-%m-%d %H:%M:%S')}。"
        )
        ...
        if not player_info.data.status.main_stage_progress:
            ...
            player_info.data.status.main_stage_progress
            in game_data.raw_data.excel.stage_table.stages
        ):
            作战进度字符串 = game_data.raw_data.excel.stage_table.stages[
                player_info.data.status.main_stage_progress
            ].code
        else:
        ...
        if player_info.data.status.subscription_end <= 0:
            ...
            lines.append(
                f"月卡{format_time(player_info.data.current_ts, player_info.data.status.subscription_end, CST, seconds=True)}到期"
            )

        # storeTs
        lines.append(
            f"storeTs：{datetime.fromtimestamp(player_info.data.status.store_ts, tz=CST).strftime('%Y-%m-%d %H:%M:%S')}"
        )

        # 最近登录时间
        lines.append(
            f"最近登录于 {format_time(player_info.data.current_ts, player_info.data.status.last_online_ts, CST, seconds=True, prefix_future='', prefix_past='')}"
        )
        ...
        # 助战干员
        lines.append(
            f"助战干员：{'、'.join(player_info.data.char_info_map[char.char_id].name for char in player_info.data.assist_chars)}"
        )

        # 已拥有干员
        lines.append(
            f"已拥有干员：{sum(1 for skl_char in player_info.data.chars if not game_data.characters.by_id(skl_char.char_id).is_patch_char)} / {sum(1 for character in game_data.characters.values() if not character.is_patch_char)}"
        )
        ...
        lines.append("")
        lines.append(
            f"日常任务：{player_info.data.routine.daily.current} / {player_info.data.routine.daily.total}"
        )
        lines.append(
            f"周常任务：{player_info.data.routine.weekly.current} / {player_info.data.routine.weekly.total}"
        )
        ...
        for char in control.chars:
            lines.append(
                f"　{player_info.data.char_info_map[char.char_id].name}（心情 {char.ap / 360000:.2f}，已工作 {format_positive_time_delta(timedelta(seconds=char.work_time))}）"
            )

        # 制造站
        for manufacture in player_info.data.building.manufactures:
            lines.append("")
            manufact_formula = game_data.raw_data.excel.building_data.manufact_formulas[
                manufacture.formula_id
            ]
            item_name = game_data.items.by_id(manufact_formula.item_id).name
            lines.append(
                f"{manufacture.level} 级制造站 {门牌号[manufacture.slot_id]}（{item_name}）"
            )
            lines.append(
                f"{format_time(player_info.data.current_ts, manufacture.complete_work_time, CST)}结束工作"
            )
            lines.append(f"容量：{manufacture.weight} / {manufacture.capacity}")
            lines.append(
                f"已完成 / 剩余：{manufacture.complete} / {manufacture.remain}"
            )
        ...
        for trading in player_info.data.building.tradings:
            lines.append("")
            lines.append(
                f"{trading.level} 级贸易站 {门牌号[trading.slot_id]}（{game_data.raw_data.excel.building_data.trading_order_des_dict[trading.strategy]}）"
            )
            lines.append(
                f"{format_time(player_info.data.current_ts, trading.complete_work_time, CST)}结束工作"
            )
        ...
        if player_info.data.building.meeting is not None:
            ...
            lines.append(
                f"结束工作时间：{format_time(player_info.data.current_ts, meeting.complete_work_time, CST, seconds=True)}"
            )
        ...
        if player_info.data.building.hire is not None:
            ...
            lines.append(
                f"结束工作时间：{format_time(player_info.data.current_ts, hire.complete_work_time, CST, seconds=True)}"
            )
        ...
        for dormitory in player_info.data.building.dormitories:
            ...
            for char in dormitory.chars:
                lines.append(
                    f"　{player_info.data.char_info_map[char.char_id].name}（心情 {char.ap / 360000:.2f}，已休息 {format_positive_time_delta(timedelta(seconds=char.work_time))}）"
                )
        ...
        for index, recruit_item in enumerate(player_info.data.recruit, start=1):
            match recruit_item.state:
                ...
                case 2:
                    lines.append(
                        f"公开招募 {index}：开始于 {format_time(player_info.data.current_ts, recruit_item.start_ts, CST, prefix_future='', prefix_past='')}，结束于 {format_time(player_info.data.current_ts, recruit_item.finish_ts, CST, prefix_future='', prefix_past='')}"
                    )
        ...
        lines.append("")
        lines.append(
            f"保全派驻每周报酬（数据增补仪）：{player_info.data.tower.reward.higher_item.current} / {player_info.data.tower.reward.higher_item.total}"
        )
        lines.append(
            f"保全派驻每周报酬（数据增补条）：{player_info.data.tower.reward.lower_item.current} / {player_info.data.tower.reward.lower_item.total}"
        )
    ...
    lines.append(
        f"森空岛信息刷新时间：{format_time(player_info.data.current_ts, player_info.data.building.labor.last_update_time, CST, prefix_future='', prefix_past='', seconds=True)}"
    )
    lines.append("")
