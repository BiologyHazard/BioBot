...
solvable_probability: defaultdict[str, float] = defaultdict(
    lambda: DEFAULT_SOLVABLE_PROBABILITY
)
...
async def start_game_func(
    event: MessageEvent, message: Annotated[Message, CommandArg()]
) -> NoReturn:
    try:
...
async def set_solvable_probability_func(
    event: MessageEvent, message: Annotated[Message, CommandArg()]
) -> NoReturn:
    try:
