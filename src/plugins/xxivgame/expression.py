...
class Expression:
    ...
    def __str__(self) -> str:
        ...
        if (self.__op in (MUL, DIV)) and (
            self.__expr0.__op in (ADD, SUB)
        ):  # (x ± y) * z or (x ± y) / z
            str_expr0 = f"({str_expr0})"
        if (self.__op in (MUL, DIV)) and (
            self.__expr1.__op in (ADD, SUB)
        ):  # x * (y ± z) or x / (y ± z)
            str_expr1 = f"({str_expr1})"
        if (self.__op == SUB) and (self.__expr1.__op in (ADD, SUB)):  # x - (y ± z)
            str_expr1 = f"({str_expr1})"
        if (self.__op == DIV) and (
            self.__expr1.__op in (MUL, DIV)
        ):  # x / (y * z) or x / (y / z)
            str_expr1 = f"({str_expr1})"
...
class Infix(list[Fraction | Operator]):
    def __init__(self, expr: str | Sequence) -> None:
        if isinstance(expr, str):
            ...
            super().__init__(
                x if isinstance(x, Operator) else Fraction(x) for x in expr
            )
