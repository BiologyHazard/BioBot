...
class Guess:
    ...
    def __init__(self, music: Music | None = None) -> None:
        if music is None:
        ...
        self.hints: list[str] = [
            ...
            f'这首乐曲{"没" if self.music.has_remaster else ""}有白谱',
            f'这首乐曲的 BPM 是 {self.music.bpm}'
        ]
        self.round: int = 0
        order: list[int] = random.sample(range(len(self.hints)), self.ROUNDS - 1)
        self.hints_shuffled: list[str] = [f'提示 {i+1}/{self.ROUNDS} {self.hints[order[i]]}'
    ...
    def give_hint(self) -> str | Message:
        ...
        self.round += 1
        if self.round < self.ROUNDS:
            return self.hints_shuffled[self.round]
