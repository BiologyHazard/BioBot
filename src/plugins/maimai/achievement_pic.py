...
_COMBO_PIC_PATHS: list[Path] = [
    plugin_config.pic_path / f"UI_MSS_MBase_Icon_{x}.png" for x in _COMBO_FILENAME
]
_ACHIEVEMENT_FILENAME: list[str] = [
    "D",
    "C",
    "B",
    "BB",
...
_ACHIEVEMENT_PIC_PATHS: list[Path] = [
    plugin_config.pic_path / f"UI_TTR_Rank_{x}.png" for x in _ACHIEVEMENT_FILENAME
]
_SYNC_FILENAME: list[str] = ["", "FS", "FSp", "FSD", "FSDp"]
_SYNC_PIC_PATHS: list[Path] = [
    plugin_config.pic_path / f"UI_MSS_MBase_Icon_{x}.png" for x in _SYNC_FILENAME
]
...
async def generate_achievement_pic(
    ...
    if version_han is not None:
        if (
            version_han[0] not in "舞霸"
            and "代" not in version_han
            and goal_han is not None
        ):
            diff_num: int = 4
        else:
            diff_num = 5
        ...
        all_need_to_play = [
            (music, diff) for music in Mai.music_list if diff < music.diff_num
        ]
        all_need_to_play.sort(key=lambda x: x[0].ds[x[1]], reverse=True)
    ...
    image_width: float = (
        border_left + (1 + col_spacing) * (num_per_line - 1) + border_right
    )
    image_height: float = y + border_down
    # 绘制背景图
    image: Image.Image = background_image(
        image_width * cover_pixels, image_height * cover_pixels, cover_pixels * 2
    )
    # 画logo和写字
    logo_image: Image.Image = image_resize_to(
        Image.open(plugin_config.pic_path / "BioBot/logo.png"),
        (9.73 * cover_pixels, None),
    )
    image.alpha_composite(
        logo_image, (round(-1.3 * cover_pixels), round(0.7 * cover_pixels))
    )
    header_font: ImageFont.FreeTypeFont = ImageFont.truetype(
        str(plugin_config.text_font_path), round(header_font_size * cover_pixels)
    )
    draw: ImageDraw.ImageDraw = ImageDraw.Draw(image)
    draw.text(
        (round((image_width - 2.0) * cover_pixels), round(2.3 * cover_pixels)),
        music_range + goals + "完成表",
        "black",
        header_font,
        "rm",
    )
    # 加载其他公用资源
    mask: Image.Image = Image.new(
        "RGBA",
        (
            round((1 + 2 * color_block_border) * cover_pixels),
            round((1 + 2 * color_block_border) * cover_pixels),
        ),
        "#00000080",
    )
    dx_image: Image.Image = image_resize_to(
        Image.open(plugin_config.pic_path / "UI_UPE_Infoicon_DeluxeMode.png"),
        (0.6084 * cover_pixels, None),
    )

    # 写字
    bottom_font: ImageFont.FreeTypeFont = ImageFont.truetype(
        str(plugin_config.text_font_path), round(bottom_font_size * cover_pixels)
    )
    draw.text(
        (
            round(image_width * cover_pixels / 2),
    ...
    row_font: ImageFont.FreeTypeFont = ImageFont.truetype(
        str(plugin_config.text_font_path), round(row_font_size * cover_pixels)
    )
    for x, y, text in texts:
        draw.text(
            (round(x * cover_pixels), round(y * cover_pixels)),
            text,
            "black",
            row_font,
            "lm",
    ...
    for (music, diff_index), (x, y) in zip(all_need_to_play[:show_count], coordinates):
        ...
        draw.rectangle(
            (
                round((x - 1 / 2 - color_block_border) * cover_pixels),
        ...
        #              colors[level_index])
        cover: Image.Image = (
            Image.open(await music.get_cover())
            .convert("RGBA")
            .resize((round(cover_pixels), round(cover_pixels)))
        )
        image.alpha_composite(
            cover,
            (round((x - 1 / 2) * cover_pixels), round((y - 1 / 2) * cover_pixels)),
        )
        if music.type == "DX":
            image.alpha_composite(
                dx_image,
                (
                    round((x + 1 / 2) * cover_pixels - dx_image.width),
        ...
        for _chart_info in data["verlist"]:
            if (
                str(_chart_info["id"]) == music.id
                and _chart_info["level_index"] == diff_index
            ):
        ...
        if chart_info is not None:  # 如果游玩过
            if combo is not None:
                ...
                if music_combo_index >= combo_goal_index:
                    ...
                    combo_image: Image.Image = Image.open(
                        _COMBO_PIC_PATHS[music_combo_index]
                    )
                    w, h = combo_image.size
                    combo_image: Image.Image = combo_image.resize(
                        (round(0.7 * cover_pixels), round(0.7 * cover_pixels * h / w))
                    )
                    w, h = combo_image.size
                    image.alpha_composite(
                        combo_image,
                        (
                            round(x * cover_pixels - w / 2),
                ...
                music_achievement_index: int = bisect_right(
                    achievementList, chart_info["achievements"]
                )
                if music_achievement_index >= achievement_goal_index:
                    ...
                    achievement_image: Image.Image = Image.open(
                        _ACHIEVEMENT_PIC_PATHS[music_achievement_index]
                    )
                    w, h = achievement_image.size
                    achievement_image: Image.Image = achievement_image.resize(
                        (round(1.3 * cover_pixels), round(1.3 * cover_pixels * h / w))
                    )
                    w, h = achievement_image.size
                    image.alpha_composite(
                        achievement_image,
                ...
                if music_sync_index >= sync_goal_index:
                    ...
                    sync_image: Image.Image = Image.open(
                        _SYNC_PIC_PATHS[music_sync_index]
                    )
                    w, h = sync_image.size
                    sync_image: Image.Image = sync_image.resize(
                        (round(0.7 * cover_pixels), round(0.7 * cover_pixels * h / w))
                    )
                    w, h = sync_image.size
                    image.alpha_composite(
