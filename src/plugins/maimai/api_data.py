...
async def get_player_data(
    project: T_project, payload: dict[str, Any], queryer: int | None = None
) -> dict[str, Any] | str:
    """
    获取用户数据，获取失败时返回字符串
    - `project` : 项目
    ...
    if (
        "qq" in payload
        and queryer != payload["qq"]
        and not query_privacy(payload["qq"])
    ):
    ...
    try:
        async with aiohttp.request(
            "POST",
            f"https://www.diving-fish.com/api/maimaidxprober/query/{p}",
            json=payload,
        ) as response:
...
async def get_rating_ranking_data() -> list[dict[str, Any]] | str:
    ...
    try:
        async with aiohttp.request(
            "GET", "https://www.diving-fish.com/api/maimaidxprober/rating_ranking"
        ) as resp:
            if resp.status != 200:
                return "未知错误，请联系BOT管理员"
