...
class DrawBest:
    def __init__(
        ...
        self.sd_best: list[ChartInfo] = sorted(
            sd_best, key=lambda x: x.ra, reverse=True
        )
        self.ds_best: list[ChartInfo] = sorted(
            dx_best, key=lambda x: x.ra, reverse=True
        )
    ...
    def _dxScore(self, info: ChartInfo) -> tuple[int, int]:
        value: int = (
            Mai.music_list.by_id(str(info.id), strict=True).charts[info.level].notes
        )
    ...
    def _findRaPic(self) -> str:
        n: int = bisect_right(
            [1000, 2000, 4000, 7000, 10000, 12000, 13000, 14000, 14500, 15000],
            self.rating,
        )
    ...
    def _findMatchLevel(self) -> str:
        n: int = (
            self.additional_rating
            if self.additional_rating <= 10
            else self.additional_rating + 1
        )
    ...
    async def whiledraw(self, data: list[ChartInfo], type: bool) -> None:
        ...
        dxstar = [
            Image.open(
                plugin_config.pic_path / f"UI_RSL_DXScore_Star_0{i + 1}.png"
            ).resize((20, 20))
            for i in range(3)
        ]
        ...
        for num, info in enumerate(data):
            ...
            cover = (
                Image.open(await get_music_cover(info.id))
                .convert("RGBA")
                .resize((135, 135))
            )
            version = Image.open(
                plugin_config.pic_path / f"UI_RSL_MBase_Parts_{info.type}.png"
            ).resize((55, 19))
            rate = Image.open(
                plugin_config.pic_path / f"UI_TTR_PhotoParts_{rankPic[info.rate]}.png"
            ).resize((80, 50))
            ...
            if info.fc:
                fc = Image.open(
                    plugin_config.pic_path
                    / f"UI_MSS_MBase_Icon_{comboPic[info.fc]}.png"
                ).resize((45, 45))
                self._im.alpha_composite(fc, (x + 240, y + 70))
            if info.fs:
                fs = Image.open(
                    plugin_config.pic_path / f"UI_MSS_MBase_Icon_{syncPic[info.fs]}.png"
                ).resize((45, 45))
            ...
            self._siyuanb.draw(
                x + 155, y + 20, 20, title, TITLE_COLOR[info.level], anchor="lm"
            )
    ...
    async def draw(self) -> Image.Image:
        ...
        advanced: Image.Image = Image.open(
            plugin_config.pic_path / "b40_score_advanced.png"
        )
        expert: Image.Image = Image.open(
            plugin_config.pic_path / "b40_score_expert.png"
        )
        master: Image.Image = Image.open(
            plugin_config.pic_path / "b40_score_master.png"
        )
        remaster: Image.Image = Image.open(
            plugin_config.pic_path / "b40_score_remaster.png"
        )
        logo: Image.Image = Image.open(
            plugin_config.pic_path / "BioBot/logo.png"
        ).resize((567, 172))
        dx_rating: Image.Image = Image.open(
            plugin_config.pic_path / self._findRaPic()
        ).resize((425, 80))
        ...
        MatchLevel: Image.Image = Image.open(
            plugin_config.pic_path / self._findMatchLevel()
        ).resize((128, 58))
        rating: Image.Image = Image.open(
            plugin_config.pic_path / "UI_CMN_Shougou_Rainbow.png"
        ).resize((454, 50))
        ...
        if self.plate:
            plate = Image.open(plugin_config.pic_path / f"{self.plate}.png").resize(
                (1420, 230)
            )
        else:
            plate = Image.open(plugin_config.pic_path / "UI_Plate_000011.png").resize(
                (1420, 230)
            )
        self._im.alpha_composite(plate, (390, 100))
        icon = Image.open(plugin_config.pic_path / "UI_Icon_0000.png").resize(
            (214, 214)
        )
        ...
        if self.qqid:
            ...
            self._im.alpha_composite(
                Image.new("RGBA", (203, 203), (255, 255, 255, 255)), (404, 114)
            )
            self._im.alpha_composite(
                qqLogo.convert("RGBA").resize((201, 201)), (405, 115)
            )
        ...
        for n, i in enumerate(f"{self.rating:05d}"):
            if i == 0:
                continue
            self._im.alpha_composite(
                Image.open(plugin_config.pic_path / f"UI_NUM_Drating_{i}.png"),
        ...
        self._siyuan.draw(
            ...
            296,
            25,
            f"历史版本: {_sd_rating} + DX 2023: {_dx_rating}",
            (0, 0, 0, 255),
            "mm",
            3,
            (255, 255, 255, 255),
        ...
        self._meiryo.draw(
            ...
            2365,
            35,
            "Designed by Yuri-YuzuChaN & BlueDeer233 | Generated by BioBot",
            (103, 20, 141, 255),
            "mm",
            3,
            (255, 255, 255, 255),
