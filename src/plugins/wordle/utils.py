...
dict_list: list[str] = [
    ...
    if f.suffix == ".json" and f.stem != "legal_words"
]

legal_words: list[list[str]] = json.loads(
    (words_dir / "legal_words.json").read_text(encoding="utf-8")
)
...
def random_word(
    dict_name: str = "CET4", word_length: int = 5
) -> tuple[str, dict[str, str]]:
    if dict_name not in dict_list:
        raise ValueError(f"dict name '{dict_name}' not in dic_list")
    if dict_name not in dicts:
        dicts[dict_name] = json.loads(
            (words_dir / f"{dict_name}.json").read_text(encoding="utf-8")
        )

    word: str = random.choice(
        list(word for word in dicts[dict_name] if len(word) == word_length)
    )
