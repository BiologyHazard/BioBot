...
def get_list_text() -> str:
    ...
    for plugin in plugin_set:
        ...
        if plugin.metadata:
            ...
            # version
            if (
                "version" in plugin.metadata.extra
                and isinstance(plugin.metadata.extra["version"], str)
                and plugin.metadata.extra["version"]
            ):
...
例如：{default_start}help help  # 获取本插件帮助"""
        ...
        if plugin is None:
            await helper.send(
                f'{arg_str} 不存在或未加载，请使用 "{default_start}help" 查看已加载的插件列表'
            )
        else:
            lines: list[str] = []
            ...
            if index > 1:
                ...
                lines.extend(
                    [f"{key}: {value}" for key, value in infos.items() if key and value]
                )
            result = "\n".join(lines)
            await helper.send(result, at_sender=True)
