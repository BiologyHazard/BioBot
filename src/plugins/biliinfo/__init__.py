...
show_info = on_regex(
    r"BV1\w{9}|b23\.(?:tv|wtf)\/\w{7}|av\d+",
    flags=re.RegexFlag.IGNORECASE + re.RegexFlag.ASCII,
)


@show_info.handle()
async def show_info_func(message: Annotated[str, EventPlainText()]):
    ...
    groups: list[T_group] = re.findall(
        r"(?:b23\.(?:tv|wtf)\/)?(?:(BV1\w{9})|(av\d+))|(b23\.(?:tv|wtf)\/\w{7})",
        message,
    )
    for group in groups[: plugin_config.biliinfo_max_count]:
        await asyncio.gather(
            show_info.send(await get_video_info(group)),
            asyncio.sleep(plugin_config.biliinfo_sleep_time),
        )
