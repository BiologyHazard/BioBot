...
type reply_dict_T = dict[str, sender_dict_T]
type group_dict_T = defaultdict[str, reply_dict_T]
type main_dict_T = defaultdict[int, group_dict_T]
...
def send_message_preprocess(message: Message) -> Message:
    ...
    for message_segment in message:
        if message_segment.type == 'image':
            message_segment.data = {'file': (plugin_config.image_folder / message_segment.data['file']).resolve().as_uri()}
    return message
...
async def download_images_from_message(message: Message, bot: Bot) -> None:
    for message_segment in message:
        if message_segment.type == 'image':
            image_info: dict[str, Any] = await bot.get_image(file=message_segment.data['file'])
...
@learn.handle()
async def learn_func(bot: Bot, event: MessageEvent, message: Message = CommandArg()) -> None:
    ...
    await load_from_file(event.group_id)
    reply_messages_dict: reply_dict_T = main_dict[event.group_id][str(trigger_message)]
...
async def forget_func(bot: Bot, event: MessageEvent, message: Message = CommandArg()) -> None:
    ...
    reply_message_dict: reply_dict_T = main_dict[event.group_id][trigger_message_str]
    if str(reply_message) not in reply_message_dict:
        await forget.finish(forget_failed_text, at_sender=True)

    sender_permission: bool = await (GROUP_OWNER | GROUP_ADMIN | SUPERUSER)(bot, event)
    if (not sender_permission) and (reply_message_dict[reply_message_str]['role'] in ['owner', 'admin']):
...
async def query_func(bot: Bot, event: MessageEvent, message: Message = CommandArg()) -> None:
    ...
    text = (f"{message} 的回复语（共 {len(main_dict[event.group_id][message_str])} 条）：\n"
            + "\n".join(f"{i}. {reply_message_str}  # 由 {sender_info['card'] or sender_info['nickname']} ({sender_info['qqid']}) 于 {datetime.fromtimestamp(sender_info['time']).strftime('%Y-%m-%d %H:%M:%S')} 设置"
                        for i, (reply_message_str, sender_info) in enumerate(main_dict[event.group_id][message_str].items(), start=1)))
...
async def query_all_func(bot: Bot, event: MessageEvent) -> None:
    ...
    text = (f'本群的全部回复语（共{len(main_dict[event.group_id])}条）\n'
            + '\n'.join(f'{i}. {trigger_message_str}  # 共 {len(main_dict[event.group_id][trigger_message_str])} 条'
...
async def reply_func(event: GroupMessageEvent, message: Message = EventMessage()) -> None:
    ...
    message_str = str(message).strip()
    message_str = str(receive_message_preprocess(Message(message_str)))
    reply_message_str = random.choice(list(main_dict[event.group_id][message_str]))
