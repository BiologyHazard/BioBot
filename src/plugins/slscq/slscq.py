...
class Slscq:
    ...
    def replace_vn(self, input_str: str) -> str:
        while input_str.find("vn") != -1:
            vn = "，".join(
                [self.get_verb() + self.get_noun() for i in range(random.randint(1, 4))]
            )
...
if __name__ == "__main__":
    paser = argparse.ArgumentParser(
        prog="slscq.py", description="auto generate a shit article"
    )
    paser.add_argument("them", help="article topic", type=str)
    paser.add_argument(
        "-n",
        "--essay_num",
        help="least character of article",
        type=int,
        default=500,
        metavar="num",
    )
