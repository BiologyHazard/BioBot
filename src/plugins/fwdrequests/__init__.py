...
def permission_checker(event: MessageEvent) -> bool:
    return (
        event.sender.user_id in forward_to_expanded
        or getattr(event, "group_id", None) in forward_to_expanded
    )


request_matcher = on_request()
approve_friend_request = on_command(
    "同意好友请求", aliases={"拒绝好友请求"}, permission=permission_checker
)
approve_group_request = on_fullmatch(
    ("同意加群请求", "拒绝加群请求"), permission=permission_checker
)
...
async def request_matcher_func(
    bot: Bot, event: FriendRequestEvent | GroupRequestEvent
) -> None:
...
async def approve_friend_request_func(
    bot: Bot,
    event: MessageEvent,
    ...
    try:
        await bot.set_friend_add_request(
            flag=latest_event.flag, approve=approve, remark=remark
        )
...
async def approve_group_request_func(
    bot: Bot, event: MessageEvent, command: Annotated[str, Fullmatch()]
) -> None:
    global latest_event, latest_user_info
    ...
    try:
        await bot.set_group_add_request(
            flag=latest_event.flag, sub_type=latest_event.sub_type, approve=approve
        )
    except Exception as e:
        logger.exception(e)
        await approve_group_request.finish(f"{text}请求失败：{e}")
    else:
        group_name: str = (await bot.get_group_info(group_id=latest_event.group_id))[
            "group_name"
        ]
